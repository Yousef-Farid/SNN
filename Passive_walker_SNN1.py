...
# =====================================================
# REAL PASSIVE WALKER SIMULATION
# =====================================================
...
# =====================================================
# EXTRACT VARIABLES TO APPROXIMATE
# =====================================================

# q1
q1_real = state_space[:, 0]

# Dq1
dq1_real = state_space[:, 2]

# q2
q2_real = state_space[:, 1]

# =====================================================
# CREATE TRAINING SIGNAL
# =====================================================

training_signal = np.column_stack([
    q1_real,
    dq1_real,
    q2_real
])
...
training_signal = training_signal[unique_indices]
...
uniform_time = np.arange(0, SIMULATION_TIME, dt)

signal_interp = np.zeros((len(uniform_time), 3))

for i in range(3):

    signal_interp[:, i] = np.interp(
        uniform_time,
        time,
        training_signal[:, i]
    )
...
def input_function(t):
    ...
    if idx >= len(signal_interp):
        idx = -1

    return signal_interp[idx]

# =====================================================
# BUILD HIGH-QUALITY SNN APPROXIMATOR
# =====================================================
...
with model:
    ...
    # =================================================
    # Large ensemble improves decoding accuracy
    # =================================================

    ensemble = nengo.Ensemble(
        n_neurons=12000, # 20000
        dimensions=3,
        radius=2,
    ...
    probe_real = nengo.Probe(input_node)

    probe_snn = nengo.Probe(decoded, synapse=0.005) # 0.02
...
real_signal = sim.data[probe_real]
snn_signal = sim.data[probe_snn]
...
real_plot = real_signal[warmup:]
snn_plot = snn_signal[warmup:]
...
plt.title("Passive Walker Approximation using SNN")
